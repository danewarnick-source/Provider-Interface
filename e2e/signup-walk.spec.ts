...
test.describe("new-provider signup walk", () => {
  test("plus-alias, list plan, optional training, TEST MODE", async ({ page }) => {
    ...
    const staffCount = page.getByTestId("signup-staff-count");
    const clientCount = page.getByTestId("signup-client-count");
    await expect(staffCount).toHaveValue("8");
    await expect(clientCount).toHaveValue("12");
    await staffCount.click();
    await staffCount.press("Control+A");
    await staffCount.press("Backspace");
    await expect(staffCount).toHaveValue("");
    await staffCount.pressSequentially("3", { delay: 20 });
    await expect(staffCount).toHaveValue("3");
    await clientCount.click();
    await clientCount.press("Control+A");
    await clientCount.press("Backspace");
    await expect(clientCount).toHaveValue("");
    await clientCount.pressSequentially("5", { delay: 20 });
    await expect(clientCount).toHaveValue("5");
    await clientCount.press("Control+A");
    await clientCount.press("Backspace");
    await clientCount.pressSequentially("12", { delay: 20 });
    await expect(clientCount).toHaveValue("12");
    await staffCount.blur();
    await clientCount.blur();
