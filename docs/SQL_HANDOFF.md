...
**What you'll see:** `ALTER TABLE` × 4, `CREATE INDEX`, `CREATE TABLE`, grants, then 116 `pack_changelog` added rows for `UT-2026.07` (ON CONFLICT DO NOTHING).
...
creates a **new** agency (payment, team size). The database already had
`accept_invitation(token)` — the app just never called it. This SQL does not
replace that RPC with a second invite system. It:

1. Stops `handle_new_user()` from creating a personal "{email}'s workspace"
   when the account was created via an invitation or Add manually.
2. Restores the All Staff group add + duplicate-invite revoke that a later
   replace of `accept_invitation` dropped.
3. Turns off a leftover personal-workspace membership if one was created in
   the last two hours (so a tester who signed up from an old link still joins
   the inviting org instead of their own empty company).
4. Lets invitations use admin, manager, or employee (the Employees invite
   dialog already offers those).

**Run this** so live invite-accept matches the `/join` page in this change.
Matches migration `supabase/migrations/20260827183000_invite_join_accept_flow.sql`.
...
**What you'll see:** two `CREATE FUNCTION`s, `REVOKE`/`GRANT`, and `ALTER TABLE`
(drop + add the role check). No rows change.
...
FROM (
  SELECT 'accept_invitation exists=' ||
    (SELECT count(*)::text FROM pg_proc p
      JOIN pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname='public' AND p.proname='accept_invitation') AS x
  UNION ALL
  SELECT 'invitations_role_check=' ||
    coalesce((SELECT pg_get_constraintdef(c.oid)
                FROM pg_constraint c
                JOIN pg_class t ON t.oid = c.conrelid
                JOIN pg_namespace n ON n.oid = t.relnamespace
               WHERE n.nspname='public' AND t.relname='invitations'
                 AND c.conname='invitations_role_check'), 'MISSING')
) s;
```
...
**Status (2026-08-27):** Compass / Cedar voice was **removed from the product**. Keep these columns — they are harmless note fields used by punch-pad / daily-log dictation. Do **not** DROP `original_transcript`. Do **not** drop `evv_timesheets`. Historical `created_from` values (including voice/compass) stay on existing rows.

**What this was for:** persist verbatim staff speech alongside the edited note. Until this column exists, only the submitted text is saved (`evv_timesheets.shift_note_text` / `daily_logs.narrative`). This adds one nullable `original_transcript` column on each note table.
...
COMMENT ON COLUMN public.evv_timesheets.original_transcript IS
  'Verbatim staff speech (or pre-edit shorthand). Never overwritten by shift_note_text.';

ALTER TABLE public.daily_logs
  ADD COLUMN IF NOT EXISTS original_transcript text;

COMMENT ON COLUMN public.daily_logs.original_transcript IS
  'Verbatim staff speech (or pre-edit shorthand). Never overwritten by narrative.';
```
...
## ACTION — HIPAA security hardening: caseload RLS, storage, PHI access audit (2026-08-25)

**What this is for:** Critical + High security controls:
1. Harden `clients_for_staff` so callers cannot dump another staff caseload
2. Add `can_access_client_phi` / `staff_assigned_to_client` and caseload-scope SELECT on `clients`, `client_medications`, `emar_logs`, `evv_timesheets`, `client_documents`
3. Set `client-photos` / `incident-photos` / `hrc-documents` buckets to **private** and org-folder-scoped storage policies
4. Create append-only `phi_access_audit_log` (service-role inserts only; admins read)

Matches migration `supabase/migrations/20260825020000_hipaa_security_hardening.sql`.

**Clear the editor, then paste this entire block:**
...
DROP POLICY IF EXISTS "caseload or admin read clients" ON public.clients;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'DROP POLICY IF EXISTS "caseload or admin read client_documents" ON public.client_documents';
    EXECUTE $p$
...
**What you'll see:** function replaces, policy drops/creates, bucket updates, one new table, RLS policies. Some `DROP POLICY IF EXISTS` may report "does not exist" — that is fine.

**Confirm (clear editor, paste next):**

```sql
SELECT string_agg(x, E'\n' ORDER BY x) AS checks
FROM (
  SELECT 'clients_for_staff=' ||
    (SELECT prosecdef::text FROM pg_proc WHERE proname = 'clients_for_staff' LIMIT 1) AS x
  UNION ALL
  SELECT 'can_access_client_phi=' ||
    (SELECT count(*)::text FROM pg_proc WHERE proname = 'can_access_client_phi')
  UNION ALL
  SELECT 'client_photos_public=' ||
    (SELECT public::text FROM storage.buckets WHERE id = 'client-photos')
  UNION ALL
  SELECT 'incident_photos_public=' ||
    coalesce((SELECT public::text FROM storage.buckets WHERE id = 'incident-photos'), 'missing')
  UNION ALL
  SELECT 'hrc_documents_public=' ||
    coalesce((SELECT public::text FROM storage.buckets WHERE id = 'hrc-documents'), 'missing')
  UNION ALL
  SELECT 'phi_access_audit_log=' ||
    (SELECT count(*)::text FROM information_schema.tables
      WHERE table_schema='public' AND table_name='phi_access_audit_log')
  UNION ALL
  SELECT 'clients_select_policies=' ||
...
You should see `can_access_client_phi=1`, `client_photos_public=false`, `incident_photos_public=false`, `hrc_documents_public=false`, `phi_access_audit_log=1`, and clients SELECT including `caseload or admin read clients`.
