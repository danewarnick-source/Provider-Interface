...
describe("DHHS91172 catalog coverage", () => {
  it("covers every imported parent and treats elements as children, not tasks", () => {
    ...
    assert.equal(report.counts.executable, 340);
    ...
    assert.equal(report.counts.wiredNineteenthBatch, 25);
    assert.equal(report.counts.wiredTwentiethBatch, 26);
    assert.equal(report.counts.wired, 340);
    ...
    assert.match(markdown, /None\. All 340 live-key parents have a fixture overlay/);
    ...
    assert.equal(holdOutChange.liveKey, "medicaid_provider_training_offered");
    assert.equal(holdOutChange.canActivate, false);
    ...
    assert.equal(vagueComply.liveKey, "ol_capacity_comply");
    assert.equal(vagueComply.canActivate, false);
    const upiRegistry = report.rows.find((r) => r.requirementKey === "REQ-30.8.1");
    assert.ok(upiRegistry);
    assert.equal(upiRegistry.liveKey, "upi_employee_registry");
    assert.equal(upiRegistry.canActivate, false);
    ...
    assert.ok(sjpQual);
    assert.equal(sjpQual.liveKey, "sjp_requires_sjd");
    assert.equal(sjpQual.canActivate, false);
    const dnr = report.rows.find((r) => r.requirementKey === "REQ-1.10.14");
    assert.ok(dnr);
    assert.equal(dnr.liveKey, "dnr_order_access");
    assert.equal(dnr.canPublish, true);
    assert.equal(dnr.canActivate, false);
    assert.equal(dnr.publication, "not_published");
    const pcsp = report.rows.find((r) => r.requirementKey === "REQ-1.24.7");
    assert.ok(pcsp);
    assert.equal(pcsp.liveKey, "pcsp_person_review");
    const fatality = report.rows.find((r) => r.requirementKey === "REQ-1.26");
    assert.ok(fatality);
    assert.equal(fatality.liveKey, "fatality_notification");
    const scTimesheet = report.rows.find((r) => r.requirementKey === "REQ-1.31.3");
    assert.ok(scTimesheet);
    assert.equal(scTimesheet.liveKey, "sc_records_on_request");
    const groupMix = report.rows.find((r) => r.requirementKey === "REQ-1.32.b");
    assert.ok(groupMix);
    assert.equal(groupMix.liveKey, "residential_group_mix");
    const usdc = report.rows.find((r) => r.requirementKey === "REQ-1.36.a");
    assert.ok(usdc);
    assert.equal(usdc.liveKey, "usdc_transition");
    const elsSchool = report.rows.find((r) => r.requirementKey === "REQ-10.3.3");
    assert.ok(elsSchool);
    assert.equal(elsSchool.liveKey, "els_school_age");
    const hostChange = report.rows.find((r) => r.requirementKey === "REQ-11.3.8");
    assert.ok(hostChange);
    assert.equal(hostChange.liveKey, "host_contractor_change");
    const hostAge = report.rows.find((r) => r.requirementKey === "REQ-11.6");
    assert.ok(hostAge);
    assert.equal(hostAge.liveKey, "host_staff_qualifications");
    const pbaMonthly = report.rows.find((r) => r.requirementKey === "REQ-15.2.6");
    assert.ok(pbaMonthly);
    assert.equal(pbaMonthly.liveKey, "pba_monthly_fiduciary");
    const form930 = report.rows.find((r) => r.requirementKey === "REQ-21.3.4");
    assert.ok(form930);
    assert.equal(form930.liveKey, "rhs_form_930");
    const form929 = report.rows.find((r) => r.requirementKey === "REQ-23.3.1");
    assert.ok(form929);
    assert.equal(form929.liveKey, "form_929_exceptional_care");
    const secPass = report.rows.find((r) => r.requirementKey === "REQ-27.3");
    assert.ok(secPass);
    assert.equal(secPass.liveKey, "sec_pass_documentation");
    const seeTrain = report.rows.find((r) => r.requirementKey === "REQ-29.4.1");
    assert.ok(seeTrain);
    assert.equal(seeTrain.liveKey, "see_staff_training");
    const seiTerm = report.rows.find((r) => r.requirementKey === "REQ-30.3.3");
    assert.ok(seiTerm);
    assert.equal(seiTerm.liveKey, "sei_job_termination");
    const tfb = report.rows.find((r) => r.requirementKey === "REQ-36.4.1");
    assert.ok(tfb);
    assert.equal(tfb.liveKey, "tfb_staff_qualifications");
    const personRecords = report.rows.find((r) => r.requirementKey === "REQ-1.10");
    assert.ok(personRecords);
    assert.equal(personRecords.liveKey, "person_record_file");
    assert.equal(personRecords.canActivate, false);
    const homeCondition = report.rows.find((r) => r.requirementKey === "REQ-11.3.2");
    assert.ok(homeCondition);
    assert.equal(homeCondition.liveKey, "home_condition_checklist");
    const grandfatherDate = report.rows.find((r) => r.requirementKey === "REQ-1.32.a");
    assert.ok(grandfatherDate);
    assert.equal(grandfatherDate.liveKey, "group_service_review_historical");
    const staffAge = report.rows.find((r) => r.requirementKey === "REQ-1.5");
    assert.ok(staffAge);
    assert.equal(staffAge.liveKey, "staff_minimum_age");
    const licenseUmbrella = report.rows.find((r) => r.requirementKey === "REQ-1.4.3");
    assert.ok(licenseUmbrella);
    assert.equal(licenseUmbrella.liveKey, "agency_license_register");
    const elsGate = report.rows.find((r) => r.requirementKey === "REQ-10.5");
    assert.ok(elsGate);
    assert.equal(elsGate.liveKey, "els_residential_eligibility");
    const upiTimesheet = report.rows.find((r) => r.requirementKey === "REQ-33.7.2");
    assert.ok(upiTimesheet);
    assert.equal(upiTimesheet.liveKey, "upi_employee_timesheet");
    const dlbc = report.rows.find((r) => r.requirementKey === "REQ-1.32.c");
    assert.ok(dlbc);
    assert.equal(dlbc.liveKey, "dlbc_group_variance");
