        ...
        <section className="rounded-xl border border-[#fed7aa] bg-gradient-to-r from-[var(--hive-text)] to-[#1a2a5a] p-5 text-white shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex min-w-[16rem] flex-col items-stretch gap-2 sm:items-end">
              <span className="inline-flex items-center gap-2 rounded-full border border-[#fed7aa] bg-[var(--hive-text)] px-3 py-1.5 text-xs font-medium text-[#fed7aa]">
...
function MetricCard({ icon, label, value, sub, warn }: { icon: React.ReactNode; label: string; value: React.ReactNode; sub?: string; warn?: boolean }) {
  return (
    <div className={`rounded-xl border p-4 shadow-sm ${warn ? "border-[#fecaca] bg-[#fef2f2]" : "border-border bg-card"}`}>
      ...
      <div className={`mt-1 font-display text-2xl font-bold ${warn ? "text-[#b91c1c]" : "text-[var(--hive-text)]"}`}>{value}</div>
  ...
  return (
    <Link
      ...
      className={`flex items-center justify-between rounded-lg border p-3 transition-colors ${
        showEmphasis
          ? "border-[var(--hive-gold)] bg-gradient-to-r from-[#fff7ed] to-[#ffedd5] hover:border-[#c2610e]"
          : "border-border bg-background hover:bg-muted"
      }`}
    >
      <div className="flex items-center gap-2 text-sm">
        <span className={showEmphasis ? "text-[var(--hive-gold)]" : "text-muted-foreground"}>{icon}</span>
      ...
      <span
        className={`inline-flex min-w-[28px] items-center justify-center rounded-full px-2 py-0.5 text-xs font-bold ${
          count === 0 ? "bg-muted text-muted-foreground" : showEmphasis ? "bg-[var(--hive-gold)] text-white" : "bg-[var(--hive-text)] text-white"
        }`}
      >
        {count}
      </span>
