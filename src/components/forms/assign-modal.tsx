...
export function AssignModal({
  ...
  const filteredClients = useMemo(() => {
    const list = (
      (data?.clients ?? []) as Array<{ id: string; first_name: string; last_name: string }>
    )
      .slice()
    ...
    return list.filter((p) =>
      `${p.first_name ?? ""} ${p.last_name ?? ""}`.toLowerCase().includes(n),
    );
  }, [data, qc]);
  ...
  function toggleC(id: string) {
    setC((arr) => (arr.includes(id) ? arr.filter((x) => x !== id) : [...arr, id]));
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <section>
            <p className="text-sm font-semibold mb-1.5">Team members — by group / billing code</p>
            <div className="flex flex-wrap gap-1.5">
              <Pill
                checked={g.includes("all_staff")}
                label="All team members"
                onClick={() => toggleG("all_staff")}
              />
              {types.map((t: { key: string; label: string }) => (
                <Pill
                  key={t.key}
                  checked={g.includes(t.key)}
                  label={t.label}
                  onClick={() => toggleG(t.key)}
                />
              ))}
              {types.length === 0 && (
                <p className="text-xs text-muted-foreground">
                  No team member types defined yet — assign individuals below.
                </p>
              )}
            </div>
          </section>
          <section>
            <p className="text-sm font-semibold mb-1.5">
              Team members — by individual ({u.length} selected)
            </p>
            <Input
            ...
            <div className="max-h-48 overflow-y-auto rounded-md border border-border divide-y">
              {filtered.map((p) => (
                <label
                  key={p.id}
                  className="flex items-center gap-2 px-3 py-2 text-sm cursor-pointer hover:bg-muted/40 min-h-[44px]"
                >
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {!allC && (
                <span className="text-xs text-muted-foreground self-center">
                  …or pick specific clients below ({c.length} selected)
                </span>
              )}
            </div>
            {!allC && (
