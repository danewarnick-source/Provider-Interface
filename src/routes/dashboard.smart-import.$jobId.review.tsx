...
import {
  ...
  saveBillingCodeRow,
  saveManualReviewRow,
  removeExtractedField,
  restoreExtractedField,
  getJobAssigner,
  upsertManualAssignment,
  removeAssignmentMapRow,
...
const CLIENT_FIELDS = [
  ...
  "emergency_contact_2_instructions",
  ...
  "pcsp_goal",
  "client_medication",
  "pcsp_has_medications",
];
type SubjectRow = {
  id: string;
  display_name: string;
  subject_type: "client" | "employee";
...
function ReviewPage() {
  ...
  const job = useQuery({
    queryKey: ["smart-import-review", jobId],
  ...
  useEffect(() => {
    if (!selectedId && job.data?.subjects?.length) {
      const next =
        job.data.subjects.find((s: SubjectRow) => s.review_status !== "ready") ??
        job.data.subjects[0];
  ...
  const onQueueSelect = (row: QueueRow) => {
    if (row.source === "other") {
      navigate({
        ...
        params: { jobId: row.import_job_id },
      });
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="gap-1">
            <Sparkles className="h-3 w-3" /> NECTAR review
          </Badge>
        </div>
      </div>
          ...
          {selectedId ? (
            <SubjectReview
              ...
              commitDisabled={
                commitCtx.isPending ||
                ready === 0 ||
                (job.data.job.source === "white_glove" && !job.data.job.provider_signoff_at)
              }
              commitReason={
                job.data.job.source === "white_glove" && !job.data.job.provider_signoff_at
                  ? "Waiting for provider sign-off"
                  : undefined
              }
            />
          ) : (
...
function AttributionBar() {
  ...
  return (
    <div className="flex items-start gap-2 rounded-xl border border-primary/20 bg-primary/5 p-3 text-sm">
      ...
      <div>
        Reviewing as <strong>{user?.email ?? "admin"}</strong> · Company Admin. Confirming approves
        this setup, including anything you changed or switched on; recorded with every item.
      </div>
    </div>
  );
}
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)] sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="mt-1 text-base font-semibold">
            {total} {noun}
            {total === 1 ? "" : "s"} · <span className="text-emerald-600">{ready} ready</span> ·{" "}
            <span className="text-amber-600">{needReview} need review</span>
          </div>
          <p className="mt-1 text-xs text-muted-foreground">
            Advisory throughout — flags surface to act on, never block.
          </p>
        </div>
        <Button
          onClick={commit}
          ...
          size="lg"
          title={whiteGlove && !signedOff ? "Waiting for provider sign-off" : undefined}
        >
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-sm font-semibold text-amber-900 dark:text-amber-100">
                {partial.length} {noun}
                {partial.length === 1 ? "" : "s"} still need attention
              </div>
...
function WhiteGloveBanner({
  ...
  const m = useMutation({
    ...
    onSuccess: () => {
      toast.success("Sign-off recorded. Commit unlocked.");
      onChanged();
    },
    onError: (e: Error) => toast.error(e.message),
  });
    ...
    <div className="flex flex-col gap-2 rounded-xl border border-amber-400/50 bg-amber-50/40 p-3 text-sm dark:bg-amber-950/30 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-start gap-2">
        ...
        <div className="text-amber-900 dark:text-amber-100">
          <strong>White-glove migration.</strong> PI staff can prep this job, but commit is locked
          until the receiving company's admin signs off below.
        </div>
      </div>
      <Button
        size="sm"
    ...
    >
      <div className="min-w-0">
        ...
        <div className="mt-0.5 flex items-center gap-1 text-[11px] text-muted-foreground">
          ...
          {row.match_status === "matched_existing" && (
            <Badge variant="outline" className="ml-1 h-4 px-1 text-[10px]">
              match
            </Badge>
          )}
          {row.match_status === "ambiguous" && (
            <Badge variant="outline" className="ml-1 h-4 px-1 text-[10px] text-amber-600">
              ambig
            </Badge>
          )}
          {row.source === "other" && (
...
function SubjectReview({
  ...
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["smart-import-subject", subjectId],
    queryFn: () => getSubj({ data: { subjectId } }),
  });
  ...
  const mergeFlags =
    (q.data as { mergeFlags?: Array<Record<string, string | number | boolean | null>> })
      .mergeFlags ?? [];
  ...
  const visibleIssues = (validation?.issues ?? []).filter(
    (i) =>
      !validationOverrides[i.key] &&
      !/^code\.(confirm_owner|coordination|coordination_info|bill_as_ours|ignore)\./.test(i.key),
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {mergeFlags.length > 0 && <MergeFlagsPanel flags={mergeFlags} onChanged={refresh} />}
...
function SubjectWizard({
  ...
  const goalFields = fields.filter(
    (f) =>
      f.target_field === "pcsp_goal" ||
      f.field_key === "pcsp_goal" ||
      f.target_field === "pcsp_goal_extraction_failed",
  );
  const medicationFields = fields.filter(
    (f) =>
      f.target_field === "client_medication" ||
      f.field_key === "client_medication" ||
      f.target_field === "pcsp_has_medications",
  );
  const hiddenProfileFields = new Set(["pcsp_goal", "client_medication", "pcsp_has_medications"]);
  const otherFields = fields.filter(
    (f) =>
      !PERSON_FIELDS_SET.has(f.target_field) &&
      !HEALTH_FIELDS_SET.has(f.target_field) &&
      !SERVICES_FIELDS_SET.has(f.target_field) &&
      !hiddenProfileFields.has(f.target_field) &&
      !f.is_custom_attribute,
  );
  // Anything we couldn't bucket falls into Person so nothing disappears.
  const personFieldsAll = [
    ...personFields,
    ...otherFields,
    ...fields.filter((f) => f.is_custom_attribute),
  ];
    ...
    <div className="space-y-4">
      {step === "person" && (
        <div className="space-y-3">
          {jobMode === "client" && (
            <GuardianConfirmCard subjectId={subjectId} fields={fields} onChanged={onChanged} />
          )}
          <PlacementLineup
      ...
      {step === "services" && (
        <PlacementLineup
          ...
          onChanged={onChanged}
          showBilling
          allowedTargetFields={Array.from(SERVICES_FIELDS_SET).filter(
            (f) => f !== "billing_code_row",
          )}
        />
      )}
      {step === "health" && (
        <div className="space-y-3">
          <div className="rounded-xl border border-border bg-card p-2.5 text-[11px] leading-snug text-muted-foreground shadow-[var(--shadow-card)]">
            This is the health/medical portion of the client profile preview. Confirm providers,
            diagnoses, allergies, swallowing risks, clinical alerts, human-rights/DNR flags, and
            other PCSP-pulled care details before creating the profile.
          </div>
          <PlacementLineup
            fields={healthFields}
            targetFields={targetFields}
      ...
      {step === "staff" && (
        <div className="space-y-3">
          <div className="rounded-xl border border-border bg-card p-2.5 text-[11px] leading-snug text-muted-foreground shadow-[var(--shadow-card)]">
            Assign staff and scope each one to the codes they're authorized for. Per-client training
            (Support strategies, Client-specific training) unlocks after PCSP upload.
          </div>
          <AssignmentMapPanel
            jobId={jobId}
            subjects={subjects}
        ...
        <div className="space-y-3">
          <ImportSummaryPanel
            subject={{
              id: subjectId,
              display_name:
                fields.find((f) => f.target_field === "full_name")?.value ??
                [
                  fields.find((f) => f.target_field === "first_name")?.value,
                  fields.find((f) => f.target_field === "last_name")?.value,
                ]
                  .filter(Boolean)
                  .join(" ") ??
                "This person",
          ...
          <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4 text-sm">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Everything in the outline above will be created for this{" "}
              {jobMode === "client" ? "client" : "staff member"} when you click{" "}
              <strong>Complete {jobMode === "client" ? "client" : "staff"} setup</strong> at the top
              of this page. Open flags become "Needed" items on the person's file — only the last
              name blocks creation.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-2 pt-2">
        ...
        {idx === steps.length - 1 ? (
          ...
          >
            ...
            <Send className="mr-1 h-3.5 w-3.5" /> Complete{" "}
            {jobMode === "client" ? "client" : "staff"} setup
          </Button>
        ) : (
          <Button
            size="sm"
...
function StepRail({
  ...
  steps: Array<{ id: string; label: string; badge?: number }>;
  activeIdx: number;
        ...
        return (
          <li key={s.id} className="flex shrink-0 items-center">
            ...
            >
              ...
              {s.badge ? (
                <span className="inline-flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-amber-500/20 px-1 text-[9px] text-amber-700 dark:text-amber-400">
                  {s.badge}
                </span>
              ) : null}
            </button>
            {i < steps.length - 1 && (
              <ChevronRight className="mx-0 h-3 w-3 shrink-0 text-muted-foreground/40" />
            )}
          </li>
        );
      })}
  ...
  const m = useMutation({
    ...
    onSuccess: (res) => {
      if (res && "ok" in res && res.ok === false) {
        const blocking =
          "blocking" in res &&
          Array.isArray((res as { blocking?: Array<{ message: string }> }).blocking)
      ...
      <div className="min-w-0">
        ...
        <div className="mt-0.5 flex flex-wrap items-center gap-1 text-[10px] text-muted-foreground">
          <Badge variant="outline" className="capitalize text-[10px] px-1.5 py-0">
            {subject.match_status.replace("_", " ")}
          </Badge>
          {subject.review_decision && (
            <Badge variant="outline" className="capitalize text-[10px] px-1.5 py-0">
              {subject.review_decision.replace("_", " ")}
            </Badge>
          )}
          <Badge variant="outline" className="capitalize text-[10px] px-1.5 py-0">
            {subject.review_status.replace("_", " ")}
          </Badge>
        </div>
      </div>
      <Button
...
function getIssueHelp(key: string, onNavigateStep?: (id: WizardStepId) => void): IssueHelp {
...
function ValidationPanel({
  ...
  validation: {
    ok: boolean;
    issues: Array<{ key: string; severity: "error" | "warning"; field?: string; message: string }>;
    blocking: string[];
  };
  onChanged: () => void;
    ...
    <div className={panelClass}>
      ...
      <ul className="mt-2 space-y-2 text-sm">
        {sortedIssues.map((i) => {
          ...
          const codeMatch = i.key.match(
            /^code\.(confirm_owner|coordination|coordination_info)\.(.+)$/,
          );
          const isGuardianConfirm =
            i.key === "guardian.unknown_status" || i.key === "contradiction.guardian_self_vs_named";
          const setCodeBucket = (
            code: string,
            bucket: "bill_as_ours" | "coordination" | "ignore",
          ) => {
          ...
          return (
            <li
              key={i.key}
              className="flex flex-col gap-2 rounded-md border border-border/70 bg-background/60 px-3 py-2"
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  {isGuardianConfirm && (
                    <div className="mt-1 text-xs text-muted-foreground">
                      Choose whether this client is their own guardian. If they have a separate
                      guardian, open Person &amp; contacts and enter name + phone.
                    </div>
                  )}
                </div>
                  ...
                  {codeMatch ? (
                    <>
                      <Button
                        ...
                        variant="default"
                        disabled={m.isPending}
                        onClick={() => setCodeBucket(codeMatch[2], "bill_as_ours")}
                      >
                        Bill this (ours)
                      </Button>
                      <Button
                        ...
                        variant="outline"
                        disabled={m.isPending}
                        onClick={() => setCodeBucket(codeMatch[2], "coordination")}
                      >
                      ...
                      <Button
                        ...
                        variant="ghost"
                        disabled={m.isPending}
                        onClick={() => setCodeBucket(codeMatch[2], "ignore")}
                      >
                        File as info / ignore
                      </Button>
                    </>
                      ...
                      {!missingField &&
                        (isBlocking ? (
                          <Button
                            ...
                            variant="default"
                            disabled={m.isPending}
                            onClick={() => m.mutate({ issueKey: i.key, overridden: true })}
                          >
              ...
              {missingField && (
                <div className="flex flex-wrap items-center gap-2 rounded border border-dashed border-border/70 bg-muted/30 px-2 py-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setInlineValue((prev) => ({ ...prev, [i.key]: e.target.value }))
                    }
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
...
function MergeFlagsPanel({
  ...
  const m = useMutation({
    mutationFn: (vars: {
      flagId: string;
      action: "keep_both" | "merge_into_existing" | "replace";
    }) => resolveFn({ data: vars }),
    onSuccess: () => {
          ...
          >
            ...
            <div className="mt-1 grid grid-cols-1 gap-1 text-sm sm:grid-cols-2">
              <div>
                ...
                {String(f.existing_value ?? "—")}
              </div>
              <div>
                <span className="text-muted-foreground">Incoming:</span>{" "}
                {String(f.incoming_value ?? "—")}
              </div>
            </div>
            <div className="mt-2 flex flex-wrap gap-2">
              <Button
                ...
                variant="outline"
                disabled={m.isPending}
                onClick={() => m.mutate({ flagId: String(f.id), action: "keep_both" })}
              >
                Keep both
              </Button>
              <Button
                ...
                variant="outline"
                disabled={m.isPending}
                onClick={() => m.mutate({ flagId: String(f.id), action: "merge_into_existing" })}
              >
                Keep existing
              </Button>
              <Button
                size="sm"
                disabled={m.isPending}
                onClick={() => m.mutate({ flagId: String(f.id), action: "replace" })}
              >
                Use incoming
              </Button>
            </div>
...
function DedupBanner({
  ...
  const m = useMutation({
    mutationFn: (decision: "update" | "create_new" | "skip") =>
      setDecision({ data: { subjectId: subject.id, decision } }),
    onSuccess: () => {
    ...
    return (
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-emerald-300/50 bg-emerald-50/40 px-3 py-2 text-xs dark:bg-emerald-950/20">
        <span className="text-emerald-800 dark:text-emerald-300">
          Decision:{" "}
          <strong className="capitalize">{subject.review_decision.replace("_", " ")}</strong>.
        </span>
        <Button size="sm" variant="ghost" onClick={() => m.mutate("skip")} disabled={m.isPending}>
          Change
        </Button>
      </div>
    );
  ...
  const existingName = matched
    ? [
        (matched as Record<string, string | null>).first_name,
        (matched as Record<string, string | null>).last_name,
      ]
        .filter(Boolean)
        .join(" ") ||
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-amber-300/50 bg-amber-50/40 p-4 dark:bg-amber-950/20 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-start gap-2 text-sm">
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            {subject.match_status === "ambiguous"
              ? "Multiple possible matches — choose how to handle."
...
function PlacementLineup({
  ...
  const popoverTargets =
    allowedTargetFields && allowedTargetFields.length > 0
      ...
      <div className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
        <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              These are the required fields for a client record. Edit any value, remove a row with
              ×, or add anything NECTAR missed.
            </div>
          </div>
        ...
        <div className="space-y-2">
          {core.length === 0 && (
            <div className="text-sm text-muted-foreground">
              No required fields extracted yet — use "+ Add a field" to enter them manually.
            </div>
          )}
          {core.map((f) => (
            <FieldRowEditor
              ...
              field={f}
              targetFields={targetFields}
              matchedValue={matched ? ((matched[f.target_field] as string | null) ?? null) : null}
              showDiff={decision === "update"}
              onChanged={onChanged}
            />
          ))}
        </div>
        ...
        <div className="rounded-2xl border border-dashed border-border bg-card p-4 shadow-[var(--shadow-card)]">
          <div className="mb-3 flex items-center justify-between">
            <div className="text-sm font-semibold flex items-center gap-2">
              <FilePlus className="h-4 w-4" /> Custom attributes
            </div>
            <div className="text-xs text-muted-foreground">
              Unknown columns — kept as-is on the person.
            </div>
          </div>
          <div className="space-y-2">
            {custom.map((f) => (
      ...
      <PopoverContent align="end" className="w-80 space-y-3">
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            placeholder={
              field ? `Type the ${labelForField(field).toLowerCase()}…` : "Pick a field first"
            }
            disabled={!field}
          />
        </div>
        <div className="flex justify-end gap-2">
          ...
          <Button
            size="sm"
            disabled={!field || !value.trim() || m.isPending}
            onClick={() => m.mutate()}
          >
...
function MedicationsReviewPanel({
  subjectId,
  fields,
  onChanged,
}: {
  ...
  const meds = fields.filter(
    (f) =>
      (f.target_field === "client_medication" || f.field_key === "client_medication") &&
      !f.dismissed_at,
  );
    ...
    <div className="rounded-2xl border border-border bg-card p-3 shadow-[var(--shadow-card)]">
          ...
          <div className="text-[11px] text-muted-foreground">
            ...
            <details className="inline">
              ...
              <span className="block mt-1 text-[11px]">
                These rows create the client's active medication list used by MAR/eMAR, medication
                attestations, client-specific training, and daily care documentation.
              </span>
            </details>
          </div>
        </div>
      ...
      {(meds.length > 0 || adding) && (
        <div className="mt-2 space-y-2">
          {meds.map((f) => (
            <MedicationReviewRowEditor
              key={f.id}
              subjectId={subjectId}
              fieldId={f.id}
              initial={parseMedicationReviewRow(f)}
              onChanged={onChanged}
            />
          ))}
          {adding && (
...
function MedicationReviewRowEditor({
  ...
  const saveMut = useMutation({
    mutationFn: () =>
      save({ data: { subjectId, fieldId, targetField: "client_medication", value: row } }),
    onSuccess: () => {
      toast.success("Saved medication");
      setDirty(false);
      onChanged();
    },
    onError: (e: Error) => toast.error(e.message),
  ...
  const removeMut = useMutation({
    ...
    onSuccess: () => {
      toast.success("Removed from this import");
      onChanged();
    },
    onError: (e: Error) => toast.error(e.message),
  });
    ...
    <div className="rounded-md border border-border/60 bg-background p-2 space-y-1.5">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {dirty && (
            <Button
              ...
              disabled={saveMut.isPending || !row.name.trim()}
            >
      ...
      <div className="grid grid-cols-1 gap-1.5 md:grid-cols-[160px_1fr]">
        <MedField label="Support level">
          <Select
            value={row.support_level || "__blank"}
            onValueChange={(v) => patch("support_level", v === "__blank" ? "" : v)}
          >
        ...
        <MedField label="Support instructions">
          <Input
            className="h-8 text-xs"
            value={row.support_explanation}
            onChange={(e) => patch("support_explanation", e.target.value)}
            placeholder="How staff supports this med"
          />
        </MedField>
      </div>
    </div>
...
function parseGoal(value: string | null): GoalShape {
  ...
  try {
    ...
    return {
      ...
      behavior_plan_link:
        typeof j.behavior_plan_link === "string" ? j.behavior_plan_link : undefined,
...
function goalCompleteness(g: GoalShape): {
  filled: number;
  total: number;
  missing: string[];
  missingKeys: Set<keyof GoalShape>;
} {
    ...
    <div className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            ...
            which service codes fund each goal, how staff support it day-to-day, and what to capture
            in daily logs — this is what transitions into support strategies, client-specific
      ...
      {extractionFailed && goals.length === 0 && (
        <div className="mt-3 rounded-lg border-2 border-amber-500 bg-amber-100/70 p-3 text-xs font-medium text-amber-900 dark:bg-amber-950/40 dark:text-amber-100">
          <div className="mb-1 text-sm font-semibold">
            NECTAR could not extract goals from this PCSP.
          </div>
          The uploaded document appears to contain a goals section, but NECTAR's extraction returned
          nothing after a focused retry. This is an extraction miss — do NOT publish this client
          until goals are entered. Add them manually below, or re-upload the PCSP to re-run
          extraction.
        </div>
      )}
      {!extractionFailed && goals.length === 0 && !adding && (
        <div className="mt-3 rounded-lg border border-amber-300/60 bg-amber-50/40 p-3 text-xs text-amber-800 dark:bg-amber-950/20 dark:text-amber-200">
          No PCSP goals were found. If the PCSP includes goals, add them here so they are not
          missing from the live client profile.
        </div>
      )}
...
function GoalReviewRowEditor({
  ...
  const saveMut = useMutation({
    mutationFn: () =>
      save({ data: { subjectId, fieldId, targetField: "pcsp_goal", value: serializeGoal(goal) } }),
    onSuccess: () => {
    ...
    <div className="rounded-lg border border-border/70 bg-background/60 p-3">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${complete ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200" : "bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-200"}`}
          >
        ...
        <div className="flex items-center gap-1">
          ...
          {dirty && (
            <Button
              ...
              className="h-7"
              onClick={() => saveMut.mutate()}
              disabled={saveMut.isPending || !goal.text.trim()}
            >
              {saveMut.isPending && <Loader2 className="mr-1 h-3 w-3 animate-spin" />}Save
            </Button>
          )}
      ...
      {!complete && !expanded && (
        <div className="mt-2 rounded-md border border-amber-300/60 bg-amber-50/40 p-2 text-[11px] text-amber-800 dark:bg-amber-950/20 dark:text-amber-200">
          <b>Needs your input:</b> {completeness.missing.join(" · ")}. Click <b>Show details</b> —
          the empty fields are highlighted in amber.
        </div>
      )}
      ...
      {expanded && !complete && (
        <div className="mt-3 rounded-md border border-amber-300/60 bg-amber-50/40 p-2 text-[11px] text-amber-800 dark:bg-amber-950/20 dark:text-amber-200">
          <b>Still needed:</b> {completeness.missing.join(" · ")}. The empty fields below are
          outlined in amber.
        </div>
      )}
      ...
      {expanded && (
        <div className="mt-3 space-y-3 border-t border-border/60 pt-3">
          <div className="grid gap-3 md:grid-cols-2">
            <TextField
              label="Domain"
              placeholder="Community Living / Healthy Living / Safety / Employment"
              value={goal.domain ?? ""}
              onChange={(v) => update("domain", v)}
              isMissing={completeness.missingKeys.has("domain")}
            />
            <TextField
              label="Who's responsible"
              placeholder="Direct Support Staff, Support Coordinator, Guardian, Behaviorist…"
              value={goal.responsible_party ?? ""}
              onChange={(v) => update("responsible_party", v)}
              isMissing={completeness.missingKeys.has("responsible_party")}
            />
          </div>
          <TextField
            label="Service codes that fund / track this goal"
            placeholder="e.g. SLN, DSI, SEI"
            value={codesText}
            onChange={updateCodes}
            hint="Comma-separated DSPD codes. Drives which daily-log codes count toward this goal."
            isMissing={completeness.missingKeys.has("service_codes")}
          />
          <AreaField
            label="Why this goal exists (rationale)"
            placeholder="From the PCSP narrative or team discussion — the person-centered reason this goal matters."
            value={goal.why ?? ""}
            onChange={(v) => update("why", v)}
            isMissing={completeness.missingKeys.has("why")}
          />
          <AreaField
            label="How staff support it (day-to-day)"
            placeholder="Prompts, cues, environmental supports, task analysis, staffing ratio…"
            value={goal.supports ?? ""}
            onChange={(v) => update("supports", v)}
            isMissing={completeness.missingKeys.has("supports")}
          />
          <AreaField
            label="What staff capture in daily logs"
            placeholder="Frequency, independence level, quantity, mood, incidents — exactly what a shift note must include to show progress."
            value={goal.data_capture ?? ""}
            onChange={(v) => update("data_capture", v)}
            isMissing={completeness.missingKeys.has("data_capture")}
          />
          <div className="grid gap-3 md:grid-cols-2">
            <AreaField
              label="Related behavior plan section"
              placeholder="e.g. BSP Objective 2 (aggression replacement)"
              value={goal.behavior_plan_link ?? ""}
              onChange={(v) => update("behavior_plan_link", v)}
            />
            <AreaField
              label="Related intake / independence sources"
              placeholder="e.g. Client Independence Assessment §3; Intake Q12"
              value={goal.intake_sources ?? ""}
              onChange={(v) => update("intake_sources", v)}
            />
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <AreaField
              label="Current status / baseline"
              placeholder="Where the person is today toward this goal."
              value={goal.current_status ?? ""}
              onChange={(v) => update("current_status", v)}
            />
            <AreaField
              label="Success criteria"
              placeholder="What 'achieved' looks like, measurably."
              value={goal.success_criteria ?? ""}
              onChange={(v) => update("success_criteria", v)}
              isMissing={completeness.missingKeys.has("success_criteria")}
            />
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <AreaField
              label="Strengths"
              placeholder="Personal strengths that support this goal."
              value={goal.strengths ?? ""}
              onChange={(v) => update("strengths", v)}
            />
            <AreaField
              label="Barriers"
              placeholder="Known barriers that staff need to plan around."
              value={goal.barriers ?? ""}
  ...
  return (
    <div className="space-y-1">
      ...
      <Textarea
        ...
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className={`min-h-[64px] text-sm ${isMissing ? "border-amber-400 focus-visible:ring-amber-400" : ""}`}
      />
    </div>
  );
}
...
function BillingCodesEditor({
  ...
  const parsed: Parsed[] = rows
    ...
    .map((f) => {
      const row = parseBillingRow(f);
      return row ? { field: f, row } : null;
    })
    .filter((x): x is Parsed => x !== null);

  const orgLabel = tenant.names[0] ?? "your organization";
  const externalRows = parsed.filter(
    (p) => providerOwnership(p.row.provider_name, tenant) === "external",
  );
  const activeExternal = externalRows.filter((p) => p.row.ownership_ack !== "not_ours");
  const unresolvedExternal = activeExternal;
  const pendingCount = unresolvedExternal.filter(
    (p) => approvals[p.field.id]?.status === "pending",
  ).length;
  const approvedCount = unresolvedExternal.filter(
    (p) => approvals[p.field.id]?.status === "approved",
  ).length;

  return (
    <div
      id="billing-codes"
      className="rounded-2xl border border-border bg-card p-3 md:p-4 shadow-[var(--shadow-card)]"
    >
      <div className="mb-2 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <div className="text-[11px] leading-snug text-muted-foreground">
            Ownership shows who bills each code. Only <span className="font-medium">Ours</span>{" "}
            flows to your 520s.{" "}
            <details className="inline">
              <summary className="inline cursor-pointer text-primary underline underline-offset-2">
                Details
              </summary>
              <span className="ml-1">
                For an external code, click <span className="font-medium">Not my organization</span>{" "}
                to keep it on the record without billing responsibility, or{" "}
                <span className="font-medium">Request PI approval</span> to have PI Admin review it
                in your Inbox.
              </span>
            </details>
          </div>
          ...
          <table className="w-full text-left text-[11px]">
            <thead className="text-muted-foreground">
              <tr className="border-b border-border">
                ...
                <th className="py-1.5 px-1.5 font-medium min-w-[180px]">Provider</th>
                <th className="py-1.5 px-1.5 font-medium w-[130px]">Ownership</th>
                <th className="py-1.5 px-1.5 font-medium w-[70px]">Unit</th>
                <th className="py-1.5 px-1.5 font-medium min-w-[100px] text-right">Rate ($)</th>
                <th className="py-1.5 px-1.5 font-medium w-[90px] text-right">Annual units</th>
                <th className="py-1.5 px-1.5 font-medium w-[110px] text-right">
                  Monthly max units
                </th>
                <th className="py-1.5 px-1.5 font-medium w-[120px]">Term</th>
            ...
            <tbody>
              {parsed.map((p) => (
                <BillingRowEditor
                  ...
                  onOpenApproval={(codeValue, providerName, requestId) =>
                    setDialog({
                      fieldId: p.field.id,
                      code: codeValue,
                      providerName,
      ...
      {activeExternal.length > 0 && (
        <details className="mt-2 rounded-md border border-amber-300/60 bg-amber-50 p-2 text-[11px] text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-200">
          <summary className="cursor-pointer font-semibold">
            {activeExternal.length} outside-provider code{activeExternal.length === 1 ? "" : "s"} on
            this PCSP
            <span className="ml-1 font-normal">
              · {approvedCount} approved · {pendingCount} awaiting PI
            </span>
          </summary>
          <div className="mt-1.5">
            Provider on {activeExternal.length === 1 ? "this line" : "these lines"} does not match{" "}
            <span className="font-medium">{orgLabel}</span>. Use{" "}
            <span className="font-medium">Not my organization</span> to keep it purely
            informational, or <span className="font-medium">Request PI approval</span> to have PI
            Admin review it.
          </div>
          <div className="mt-1 font-mono text-[10px]">
            {activeExternal
              .map((p) => `${p.row.service_code} → ${p.row.provider_name ?? "unknown"}`)
              .join("  •  ")}
          </div>
        </details>
      )}
...
function BillingRowEditor({
  ...
  useEffect(() => {
    setRateText(row.rate == null ? "" : String(row.rate));
  }, [row.rate]);

  return (
    <tr
      className={`border-b border-border/60 align-middle ${notOurs ? "bg-muted/30 text-muted-foreground" : ""}`}
    >
      <td className="py-1 px-1.5">
        {isNew ? (
          <Select value={row.service_code} onValueChange={(v) => patch("service_code", v)}>
            <SelectTrigger className="h-7 w-full px-1.5 text-[11px]">
      ...
      <td className="py-1 px-1.5">
        {notOurs ? (
          <span className="text-[11px]">
            {row.provider_name ?? <span className="text-muted-foreground/60">—</span>}
          </span>
        ) : (
          <Input
            className="h-7 w-full px-1.5 text-[11px]"
            value={row.provider_name ?? ""}
            onChange={(e) => patch("provider_name", e.target.value || null)}
          />
        )}
      </td>
      <td className="py-1 px-1.5">
        {(() => {
          ...
          if (own === "ours") {
            return (
              <Badge
                variant="outline"
                className="whitespace-nowrap border-emerald-500/60 text-emerald-600 text-[10px] px-1.5 py-0"
              >
                Ours
              </Badge>
            );
          ...
          if (own === "unknown") {
            return (
              <Badge
                variant="outline"
                className="whitespace-nowrap text-muted-foreground text-[10px] px-1.5 py-0"
              >
                Unspecified
              </Badge>
            );
          ...
          if (row.ownership_ack === "not_ours") {
            ...
            return (
              <div className="flex flex-col gap-0.5">
                <Badge
                  variant="outline"
                  className="whitespace-nowrap border-slate-400/60 text-slate-600 dark:text-slate-300 text-[10px] px-1.5 py-0"
                >
                  Not our organization
                </Badge>
                <span className="text-[9px] text-muted-foreground leading-tight">
                  Informational only — not billed or tracked.
                </span>
                <button
                  type="button"
          ...
          const openDialog = () =>
            onOpenApproval(row.service_code, row.provider_name ?? null, ar?.id ?? null);
          const markNotOurs = () => {
            ...
            save({
              ...
              .then(() => {
                toast.success("Marked as not your organization");
                onChanged();
              })
              .catch((e: Error) => toast.error(e.message));
          };
          ...
          if (!ar || ar.status === "withdrawn") {
            ...
            statusEl = (
              <Badge
                variant="outline"
                className="whitespace-nowrap border-amber-500/60 text-amber-700 dark:text-amber-300 text-[10px] px-1.5 py-0"
              >
                Awaiting PI
              </Badge>
            );
            ...
            statusEl = (
              <Badge
                variant="outline"
                className="whitespace-nowrap border-emerald-500/60 text-emerald-700 dark:text-emerald-300 text-[10px] px-1.5 py-0"
              >
            ...
            statusEl = (
              <Badge
                variant="outline"
                className="whitespace-nowrap border-destructive/60 text-destructive text-[10px] px-1.5 py-0"
              >
                Denied
              </Badge>
            );
      ...
      <td className="py-1 px-1.5 text-right">
        {notOurs ? (
          ...
          <div className="relative">
            <span className="pointer-events-none absolute left-1.5 top-1/2 -translate-y-1/2 text-[11px] text-muted-foreground">
              $
            </span>
            <Input
              ...
              onBlur={() => {
                ...
                if (!Number.isFinite(n)) {
                  setRateText(row.rate == null ? "" : row.rate.toFixed(2));
                  return;
                }
      ...
      <td className="py-1 px-1.5 text-right">
        {notOurs ? (
          ...
          <Input
            className="h-7 w-full px-1.5 text-[11px] text-right"
            inputMode="numeric"
            value={row.max_units ?? ""}
            onChange={(e) => patch("max_units", numOrNull(e.target.value))}
          />
        )}
      </td>
      <td className="py-1 px-1.5 text-right">
        {notOurs ? (
          ...
          <Input
            className="h-7 w-full px-1.5 text-[11px] text-right"
            inputMode="numeric"
            value={row.monthly_max_units ?? ""}
            onChange={(e) => patch("monthly_max_units", numOrNull(e.target.value))}
          />
        )}
      </td>
      <td className="py-1 px-1.5">
        {notOurs ? (
          ...
          <div className="flex items-center gap-0.5">
            <Input
              ...
              onChange={(e) => patch("plan_start", e.target.value || null)}
              title="Start"
            />
            ...
            <Input
              ...
              type="date"
              value={row.plan_end ?? ""}
              onChange={(e) => patch("plan_end", e.target.value || null)}
              title="End"
            />
          </div>
        )}
      ...
      <td className="py-1 px-1.5">
        {notOurs ? null : pending ? (
          <Badge
            variant="outline"
            className="whitespace-nowrap text-amber-600 text-[10px] px-1.5 py-0"
          >
            <AlertTriangle className="mr-1 h-2.5 w-2.5" /> pending
          </Badge>
        ) : (
          <Badge variant="outline" className="text-emerald-600 text-[10px] px-1.5 py-0">
            ready
          </Badge>
        )}
      </td>
        ...
        <div className="flex items-center justify-end gap-0.5">
          {dirty && !notOurs && (
            <Button
              ...
              onClick={() => saveMut.mutate()}
              disabled={saveMut.isPending || !row.service_code}
            >
              {saveMut.isPending && <Loader2 className="mr-1 h-3 w-3 animate-spin" />}Save
            </Button>
          )}
          {isNew && onCancel && (
            <Button size="sm" variant="ghost" className="h-6 px-1.5 text-[10px]" onClick={onCancel}>
              Cancel
            </Button>
          )}
          {!isNew && fieldId && (
...
function FieldRowEditor({
  ...
  useRegisterDirtyFlush(
    dirty ? `field:${field.id}` : `field:${field.id}:clean`,
    dirty ? saveIfDirty : null,
  );

  const dismissM = useMutation({
    mutationFn: () => dismiss({ data: { fieldId: field.id } }),
    onSuccess: () => {
      toast.success("Dismissed — will not commit");
      onChanged();
    },
    onError: (e: Error) => toast.error(e.message),
  });
      ...
      {field.is_custom_attribute ? (
        ...
        >
          ...
          <SelectContent>
            ...
            {!targetFields.includes(target) && (
              <SelectItem value={target}>{labelForField(target)}</SelectItem>
            )}
          </SelectContent>
        </Select>
      )}
      <div className="flex flex-1 items-center gap-1.5 text-xs">
        ...
        {showDiff && !dismissed && matchedValue && matchedValue !== value && (
          <span className="text-muted-foreground">
            was: <span className="font-mono">{matchedValue}</span>
          </span>
        )}
      </div>
      {dirty && !dismissed && (
  ...
  if (questions.length === 0) {
    return (
      <div className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground shadow-[var(--shadow-card)]">
        No clarifying questions from NECTAR.
      </div>
    );
  }
  return (
    <div className="space-y-2">
      {questions.map((q) => (
        <QuestionItem key={q.id} q={q} onChanged={onChanged} />
      ))}
    </div>
  );
}
function QuestionItem({
  ...
  q: { id: string; question: string; context: string | null; answer: string | null };
  onChanged: () => void;
}) {
...
function ImportSummaryPanel({
  ...
  const missing = (v: string | null | undefined) =>
    v ? (
      ...
      <span className="text-muted-foreground/60 italic">— none captured —</span>
    );
  ...
  const oursCount = parsedBilling.filter(
    (r) => providerOwnership(r.provider_name, tenant) === "ours",
  ).length;
  const externalActive = parsedBilling.filter(
    (r) =>
      providerOwnership(r.provider_name, tenant) === "external" && r.ownership_ack !== "not_ours",
  );
  ...
  const staffAssignments = assignments.filter((a) =>
    jobMode === "client" ? a.client_subject_id === clientId : a.staff_subject_id === clientId,
  );
    ...
    return (
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          ...
          {count && (
            <span className={`rounded-full border px-1.5 py-0.5 text-[9px] font-medium ${toneCls}`}>
              {count}
            </span>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="min-w-0">
            <div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
              Final review — everything below will be created on Complete client setup
            </div>
      ...
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <div className={cardCls}>
          ...
          <Row
            ...
            value={missing([scName, scCompany].filter(Boolean).join(" · ") || null)}
          />
          <Row
            label="Guardian"
            value={
              isOwnGuardian ? (
                <span>Self / own guardian</span>
              ) : guardianName ? (
                <span>
                  {[guardianName, guardianRel, guardianPhone].filter(Boolean).join(" · ")}
                </span>
              ) : (
                missing(null)
              )
        ...
        <div className={cardCls}>
          <SectionHeader
            ...
            step="health"
            count={`${diagnoses.length} dx · ${allergies.length} allergy`}
          />
        ...
        <div className={cardCls}>
          ...
          {parsedMeds.length === 0 ? (
            <div className="text-[11px] text-muted-foreground italic">
              {hasNoMedsSignal
                ? "PCSP indicates no medications."
                : "— none captured — add manually in Step 3 if the PCSP lists any."}
            </div>
          ) : (
        ...
        <div className={cardCls}>
          <SectionHeader
            ...
            step="goals"
            tone={
              goalExtractionFailed || incompleteGoals > 0
                ? "amber"
                : parsedGoals.length
                  ? "emerald"
                  : "neutral"
          ...
          {parsedGoals.length === 0 ? (
            <div className="text-[11px] text-muted-foreground italic">
              — none captured — add manually in Step 4.
            </div>
          ) : (
            <ul className="space-y-0.5 text-[11px]">
              {parsedGoals.map((g, i) => {
                ...
                return (
                  <li key={i} className="flex items-start justify-between gap-2">
                    ...
                    <span
                      className={`shrink-0 rounded-full px-1.5 py-0 text-[9px] ${c.missing.length === 0 ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200" : "bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-200"}`}
                    >
                      {c.filled}/{c.total}
                    </span>
                  </li>
              ...
              <table className="w-full min-w-[560px] text-left text-[11px]">
                ...
                <tbody>
                  {parsedBilling.map((r, i) => {
                    ...
                    if (notOurs) {
                      statusLabel = "Not my org · informational";
                      statusCls = "text-muted-foreground";
                    } else if (own === "external") {
                      statusLabel = "External · coordination / PI";
                      statusCls = "text-amber-700 dark:text-amber-300";
                    } else if (own === "unknown") {
                      statusLabel = "Provider unspecified";
                      statusCls = "text-muted-foreground";
                    }
                    return (
                      <tr
                        key={i}
                        className={`border-b border-border/40 ${notOurs ? "bg-muted/20 text-muted-foreground" : ""}`}
                      >
                        <td className="px-1.5 py-1 font-mono">{r.service_code}</td>
                        <td className="px-1.5 py-1">
                          {r.provider_name ?? <span className="text-muted-foreground/60">—</span>}
                        </td>
                        ...
                        <td className="px-1.5 py-1 text-right">
                          {r.max_units ?? r.monthly_max_units ?? "—"}
                        </td>
                        <td className="px-1.5 py-1">
        ...
        <div className={cardCls}>
          ...
          <div className="mt-1 text-[10px] text-muted-foreground">
            Per-client training (Support strategies, Client-specific training) is created
            automatically once PCSP + goals are finalized.
          </div>
        </div>
      </div>
    </div>
...
function ProvisioningPanel({ subjectId, onChanged }: { subjectId: string; onChanged: () => void }) {
  ...
  const m = useMutation({
    mutationFn: (vars: {
      planId: string;
      state: "will_create" | "draft" | "added_by_admin" | "na";
      note?: string;
    }) => toggle({ data: { planId: vars.planId, state: vars.state, override_note: vars.note } }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["subject-forecast", subjectId] });
      onChanged();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const groupOrder: Array<{ key: string; label: string; match: (m: string) => boolean }> = [
    ...
    {
      key: "nonevv",
      label: "Time clock — non-EVV (payroll / evidence)",
      match: (m) => m === "time_clock_non_evv" || m === "time_clock",
    },
    ...
    >
      <div>
        <div className="text-sm font-medium">
          {prettifyModule(p.target_module)} ·{" "}
          <span className="text-xs uppercase tracking-wide text-muted-foreground">
            {p.planned_action.replace(/_/g, " ")}
          </span>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Select
          ...
          onValueChange={(v) =>
            m.mutate({
              planId: p.id,
              state: v as "will_create" | "draft" | "added_by_admin" | "na",
            })
          }
        >
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
      <div className="flex items-start gap-2 text-sm">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Preview only — nothing is created here. Time-clock entries are derived from this
            client's owned billing codes and split by EVV vs non-EVV. Other modules come from your
            automation rules.
          </p>
        </div>
      </div>
      <div className="mt-3 space-y-4">
        {plan.length === 0 && (
          <div className="text-sm text-muted-foreground">
            No provisioning items — no owned billing codes and no automation rules matched.
          </div>
        )}
        {grouped.map((g) => (
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        Assign staff to each client — optionally scoped to specific authorized codes (e.g. "Julie
        covers HHS only"). Staff↔client rows are written to caseloads on commit. Coordination-only
        codes from other providers aren't selectable.
      </p>

      {q.isLoading && <div className="text-xs text-muted-foreground">Loading assigner…</div>}
      {q.isError && (
        <div className="text-xs text-destructive">Failed to load: {(q.error as Error).message}</div>
      )}
        ...
        <div className="space-y-3">
          {clientSubjects.map((cs) => {
            const client = q.data.clients.find(
              (c: AssignerClient) => c.client_subject_id === cs.id,
            );
            const rows = q.data.assignments.filter(
              (a: AssignerAssignment) => a.client_subject_id === cs.id,
            );
            return (
              <ClientAssignerBlock
                key={cs.id}
...
function ClientAssignerBlock({
  ...
  const allFilteredSelected =
    filteredStaff.length > 0 && filteredStaff.every((s) => selected.has(s.id));
    ...
    <div className="rounded-lg border border-border p-3">
      ...
      {proposals.length > 0 && (
        <div className="mt-2 space-y-1 rounded border border-dashed border-border bg-muted/30 p-2 text-xs">
          <div className="font-medium text-muted-foreground">
            NECTAR proposals (subjects not yet committed)
          </div>
          {proposals.map((p) => (
            <div key={p.id} className="text-muted-foreground">
              {p.relation_type}:{" "}
              <span className="font-medium">staff subject {p.staff_subject_id?.slice(0, 6)}…</span>
      ...
      <div className="mt-2 space-y-1.5">
        ...
        {editable.map((row) => (
          <div
            key={row.id}
            className="flex flex-wrap items-center gap-2 rounded border border-border bg-background p-2 text-xs"
          >
            ...
            <AssignerScopePopover
              ...
              onChange={(codes) =>
                upsertM.mutate({ staffId: row.staff_record_id!, serviceCodes: codes })
              }
      ...
      <Dialog open={pickerOpen} onOpenChange={setPickerOpen}>
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button onClick={assignSelected} disabled={selected.size === 0 || upsertM.isPending}>
