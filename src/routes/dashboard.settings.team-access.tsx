...
const ROLE_INFO: Array<{
  ...
  { key: "company_executive", label: "Company Executive", hint: "Admin + executive/billing views" },
  {
    key: "hive_executive",
    label: "PI Executive",
    hint: "Cross-company platform tools",
    restricted: true,
  },
];
...
function TeamAccessPage() {
  ...
  const canManage = org?.role === "admin" || isHiveExec;
  ...
  const invite = useMutation({
    ...
    onSuccess: () => {
      toast.warning(
        "Invitation created — this screen does not send email. Copy the join link from Team Members or Invitations. Prefer Team Members → Add team member for a complete file, then send a join email or copy a temp password.",
      );
      setInviteEmail("");
  ...
  const canManageGroups = org?.role === "admin" || isHiveExec;
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Each login can hold any combination of these roles. Roles are additive — a single account
          switches context via the Portal View switcher.
        </p>
      </div>
      ...
      <Tabs defaultValue="members">
        <TabsList>
          ...
          <TabsTrigger value="role-history" asChild>
            <Link to="/dashboard/settings/role-audit" className="inline-flex items-center gap-1">
        ...
        <TabsContent value="members" className="space-y-6">
          ...
          >
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Prefer Team Members → Add team member for a complete file, then send a join email or
              copy a temp password. This form only writes a pending invitation row — it does not
              send email. Committee Member is assigned after they join.
            </p>
          </form>
