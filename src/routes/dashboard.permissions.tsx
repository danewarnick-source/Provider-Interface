...
const EDITABLE_ROLES: ProviderRole[] = [
  "admin",
  "program_manager",
  "manager",
  "employee",
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage role defaults and review the change history for{" "}
    ...
    <div className="space-y-6">
          ...
          return (
            ...
            >
              <CollapsibleTrigger asChild>
                <button
                  type="button"
                  className="flex w-full items-center gap-2 bg-secondary/10 p-3 text-left text-sm font-semibold"
                >
                  <ChevronDown
      ...
      <AlertDialog open={!!resetRole} onOpenChange={(v) => !v && setResetRole(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Reset {resetRole ? ROLE_LABEL[resetRole] : ""} to defaults?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This will reset all {resetRole ? ROLE_LABEL[resetRole] : ""} permissions to the
              standard defaults. Any customizations for this role will be lost. Continue?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => resetRole && doReset(resetRole)}>
              Reset
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<AuditRow[]> => {
      const { data, error } = await supabase
        ...
        .select(
          "id, created_at, changed_by_name, change_type, role, target_user_name, permission, previous_value, new_value",
        )
              ...
              <tr key={row.id} className="border-b border-border last:border-0">
                ...
                <td className="p-3">
                  {row.target_user_name
                    ? `${row.target_user_name} — individual`
                    : row.role
                      ? (ROLE_LABEL[row.role as Role] ?? row.role)
                      : "—"}
                </td>
                <td className="p-3 text-xs">
                  {PERMISSION_LABEL[row.permission as Permission] ?? row.permission}
                </td>
