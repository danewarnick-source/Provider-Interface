...
function CompanyMigrationPage() {
  ...
  const engagementStatus: EngagementStatus =
    activeJob?.engagement_status ??
    (jobsQ.data?.[0]?.engagement_status as EngagementStatus | undefined) ??
    "quoted";
  ...
  useEffect(() => {
    ...
    logFn({
      data: {
        ...
        details: { target_org_id: targetOrgId },
      },
    }).catch(() => null);
  }, [activeJob?.id]);
    ...
    <div className="space-y-5">
      <header className="space-y-1">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          Paid white-glove onboarding. PI staff prep the import using the shared NECTAR engine; the
          receiving company's admin signs off before anything commits. Customer companies never see
          this tool.
        </p>
      </header>
      ...
      <div className="flex items-start gap-2 rounded-lg border border-amber-400/50 bg-amber-50/40 p-3 text-xs dark:bg-amber-950/30">
        ...
        <div className="text-amber-900 dark:text-amber-100">
          <strong>Internal only.</strong> Customer companies never see this page. Files are uploaded
          through the platform's private bucket under the BAA — never email. Every PI-staff action
          on this customer's data is logged.
        </div>
      </div>
          ...
          <div className="space-y-1">
            ...
            <Select value={targetOrgId} onValueChange={setTargetOrgId}>
              <SelectTrigger id="target-company">
                <SelectValue
                  placeholder={companiesQ.isLoading ? "Loading companies…" : "Select a company"}
                />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-1">
            ...
            <Select value={mode} onValueChange={(v) => setMode(v as "employee" | "client")}>
              ...
              <SelectContent>
                ...
                <SelectItem value="employee">Team members</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
            ...
            <div className="flex items-center rounded-md border border-input bg-background px-2">
              ...
              <input
                ...
                onBlur={() => {
                  if (activeJob && quote) quoteM.mutate(Math.round(parseFloat(quote) * 100));
                }}
      ...
      <NectarGuidanceStrip
        ...
        message={
          <>
            This page runs the same Smart Import engine the customer uses, just scoped to their
            company. You can ingest, map, and clean. The commit only fires after their admin reviews
            and signs off — they own that decision.
          </>
        }
      />
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-sm font-semibold">
                Start a migration for {targetCompany?.name}
              </div>
              <p className="text-xs text-muted-foreground">
                Creates a white-glove import job scoped to this company. You'll be routed into the
                shared engine to upload their export (private bucket under the BAA) and prep the
                mapping.
              </p>
            </div>
            <Button
        ...
        <Card className="border-border/60 p-5">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <div className="flex items-center gap-2 text-sm font-semibold">
                <Building2 className="h-4 w-4 text-[var(--hive-gold)]" /> Active migration ·{" "}
                {targetCompany?.name}
              </div>
          ...
          {!activeJob.provider_signoff_at && (
            <div className="mt-4 flex items-start gap-2 rounded-md border border-amber-400/40 bg-amber-50/40 p-3 text-xs dark:bg-amber-950/30">
              ...
              <div className="text-amber-900 dark:text-amber-100">
                Commit is locked. The receiving company's admin must open the review screen at the
                link above and record their sign-off. PI staff cannot self-commit a customer's data.
              </div>
            </div>
          )}
        </Card>
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-muted-foreground">
                    ...
                    {j.committed_at &&
                      ` · committed ${new Date(j.committed_at).toLocaleDateString()}`}
                  </span>
                  {j.quote_amount_cents != null && (
                    <span className="text-muted-foreground">
                      · ${(j.quote_amount_cents / 100).toLocaleString()}
                    </span>
                  )}
                </div>
                <Link
