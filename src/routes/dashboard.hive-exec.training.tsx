...
function statusTimestamp(r: ExecEnrollmentRow): string {
  return r.certificate_uploaded_at ?? r.completed_at ?? r.link_sent_at ?? r.enrolled_at;
}
...
function TrainingFulfillmentPage() {
  ...
  const summary = useMemo(() => {
    ...
    return {
      needsAction: rows.filter(
        (r) =>
          r.status === "certificate_pending" ||
          (r.status === "link_sent" && daysSince(statusTimestamp(r)) > 3),
      ).length,
      awaitingTraining: rows.filter((r) => r.status === "enrolled" || r.status === "link_sent")
        .length,
      awaitingCert: rows.filter((r) => r.status === "certificate_pending").length,
      verifiedThisMonth: rows.filter(
        (r) => r.verified_at && new Date(r.verified_at).getTime() >= startOfMonth,
      ).length,
    };
  }, [rows]);
  ...
  const runBulk = async (
    action: "mark_link_sent" | "mark_completed_and_notify_admins" | "remind_admins",
  ) => {
    if (!selectedRows.length) return;
    const label =
      action === "mark_link_sent"
        ? "mark all as link sent"
        : action === "mark_completed_and_notify_admins"
          ? "mark all completed and notify admins"
          : "remind all admins";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <SummaryCard
          ...
          value={summary.verifiedThisMonth}
          accent="emerald"
        />
      </div>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Search org or staff…"
        ...
        {selectedRows.length > 0 && (
          <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-accent/40 bg-accent/5 px-3 py-2 text-xs">
            ...
            {uniformStatus === "enrolled" && (
              <Button size="sm" onClick={() => runBulk("mark_link_sent")}>
                Mark all as link sent
              </Button>
            )}
            {uniformStatus === "link_sent" && (
              <Button size="sm" onClick={() => runBulk("mark_completed_and_notify_admins")}>
                Mark all completed + notify admins
              </Button>
            )}
            {uniformStatus === "certificate_pending" && (
              <Button size="sm" onClick={() => runBulk("remind_admins")}>
                Remind all admins
              </Button>
            )}
            {!uniformStatus && (
              <span className="text-muted-foreground">
                Select rows with the same status to use bulk actions.
              </span>
            )}
          </div>
        )}
                    ...
                    <tr key={r.id} className="border-b border-border/60 hover:bg-muted/30">
                      ...
                      <td className="py-2 pr-3">
                        <RowActions
                          row={r}
                          onSendLink={async () => {
                            await linkSentFn({ data: { enrollment_id: r.id } });
                            invalidate();
                          }}
                          onMarkComplete={async () => {
                            await completedFn({ data: { enrollment_id: r.id } });
                            invalidate();
                          }}
                          onRemind={async () => {
                            await remindFn({ data: { enrollment_id: r.id } });
                            invalidate();
                          }}
...
function RowActions({
  ...
  if (row.status === "enrolled") {
    return (
      <Button size="sm" variant="outline" disabled={working} onClick={() => run(onSendLink)}>
        Send link
      </Button>
    );
  }
  if (row.status === "link_sent") {
    return (
      <div className="flex gap-1">
        <Button size="sm" variant="outline" disabled={working} onClick={() => run(onMarkComplete)}>
          Mark complete
        </Button>
        <Button size="sm" variant="ghost" disabled={working} onClick={() => run(onSendLink)}>
          Resend
        </Button>
      </div>
    );
  ...
  if (row.status === "certificate_pending") {
    return (
      <Button size="sm" variant="outline" disabled={working} onClick={() => run(onRemind)}>
        Remind admin
      </Button>
    );
  }
  ...
  if (row.status === "verified") {
    return (
      <span className="text-muted-foreground">
        {row.nectar_extracted_expires_date
          ? `Expires ${row.nectar_extracted_expires_date}`
          : "Verified"}
      </span>
    );
  }
      ...
      >
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="font-semibold">
            {row.staff_name} — {row.product_name}
          </h3>
        </div>
        <div className="space-y-2 text-sm">
          <div>
            Nectar validation:{" "}
            <span
              className={failed ? "text-destructive font-medium" : "text-emerald-600 font-medium"}
            >
              {row.nectar_validation_status ?? "—"}
            </span>
          </div>
          {row.nectar_extracted_expires_date && (
            <div>Extracted expiration: {row.nectar_extracted_expires_date}</div>
          )}
        </div>
        {failed && (
          <div className="mt-4 space-y-2 border-t border-border pt-3">
            <p className="text-xs text-muted-foreground">
              Nectar rejected this certificate. If you've reviewed it manually and it's valid,
              confirm it here.
            </p>
            <Input
              type="date"
            ...
            />
            <Button size="sm" onClick={() => onConfirmManually(expiresOn || null)}>
              Confirm manually
            </Button>
          </div>
        )}
